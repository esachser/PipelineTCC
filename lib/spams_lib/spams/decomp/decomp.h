...
template <typename T> class OMPSolver {
    public:
    ...
    _npatches(num_patches), _D(D), _L(L), _eps(eps), _lambda(lambda), _G(D, true)
{
    ...
    _tmpT=new Vector<T>[_NUM_THREADS];
    _RdnT=new Vector<T>[_NUM_THREADS];
    _UnT=new Matrix<T>[_NUM_THREADS];
    _UndnT=new Matrix<T>[_NUM_THREADS];
    _GsT=new Matrix<T>[_NUM_THREADS];
...
template <typename T> void OMPSolver<T>::solve(const Matrix<T>& X) {
    ...
    if (X.n() != _npatches){
        fprintf(stderr, "Error! Wrong number of patches. %d != %d", X.n(), _npatches);
    }
    int M = _npatches;
